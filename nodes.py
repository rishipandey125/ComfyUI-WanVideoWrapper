...
class WanVideoChainedSampler:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "seed": ("INT", {"default": 0, "min": 0, "max": 0xffffffffffffffff}),
                "force_offload": ("BOOLEAN", {"default": True}),
                "scheduler": (["unipc", "unipc/beta", "dpm++", "dpm++/beta","dpm++_sde", "dpm++_sde/beta", "euler", "euler/beta","euler/accvideo", "deis", "lcm", "lcm/beta", "flowmatch_causvid", "flowmatch_pusa"], {"default": 'unipc'}),
    ...
    def process(self, vae, model, text_embeds, width, height, num_frames, max_frames_per_chunk, reverse_processing, perfect_loop, control_frames, key_frames, reference_image, keyframe_indices, cn_strength, i2v_strength, vace_percentage, steps, cfg, shift, seed, force_offload, scheduler, riflex_freq_index, denoise_strength, batched_cfg, rope_function, overlap_frames, model_reference, color_match, color_match_method, slg_args=None, experimental_args=None):
        ...
        def run_chunk(start, end, key_frames, control_frames, forward=True, overlap_images=None):
            ...
            if overlap_images is not None:
                ...
                if not forward: 
                    ...
                    overlap_indices = list(range(num_frames - overlap_images.shape[0], num_frames))
        ...
        if num_frames <= max_frames_per_chunk:
            ...
            if b_start < max_frames_per_chunk:
                b_start = 0
            ...
            # Set f_start based on whether we went backwards or not
            if went_backwards:
                f_start = first_keyframe
            else:
                f_start = 0  # Start from beginning if no backward processing
